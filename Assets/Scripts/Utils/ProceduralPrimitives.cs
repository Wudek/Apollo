...
public static class ProceduralPrimitives {
    private static uint _boxID;
    private static uint _planeID;
    private static uint _coneID;
    private static uint _tubeID;
    private static uint _torusID;
    private static uint _sphereID;

    public static GameObject CreatePlane() {
        // You can change that line to provide another MeshFilter
        var go = new GameObject("Plane_" + ++_planeID);
    ...
    public static GameObject CreateBox() {
        var go = new GameObject("Box_" + ++_boxID);
        var filter = go.AddComponent<MeshFilter>();
        Mesh mesh = filter.mesh;
        mesh.Clear();
        float length = 1f;
...
// Bottom
    ...
    public static GameObject CreateCone() {
        var go = new GameObject("Cone_" + ++_coneID);
        var filter = go.AddComponent<MeshFilter>();
        Mesh mesh = filter.mesh;
        mesh.Clear();
        float height = 1f;
    ...
    public static GameObject CreateTube() {
        var go = new GameObject("Tube_" + ++_tubeID);
        var filter = go.AddComponent<MeshFilter>();
        Mesh mesh = filter.mesh;
        mesh.Clear();
        float height = 1f;
        ...
        while (vert < nbVerticesCap) {
            ...
            vertices[vert] = new Vector3(cos * (bottomRadius1 - bottomRadius2 * .5f), 0f,
                sin * (bottomRadius1 - bottomRadius2 * .5f));
            vertices[vert + 1] = new Vector3(cos * (bottomRadius1 + bottomRadius2 * .5f), 0f,
                sin * (bottomRadius1 + bottomRadius2 * .5f));
            vert += 2;
        }
        // Top cap
        ...
        while (vert < nbVerticesCap * 2) {
            ...
            float cos = Mathf.Cos(r1);
            float sin = Mathf.Sin(r1);
            vertices[vert] = new Vector3(cos * (topRadius1 - topRadius2 * .5f), height,
                sin * (topRadius1 - topRadius2 * .5f));
            vertices[vert + 1] = new Vector3(cos * (topRadius1 + topRadius2 * .5f), height,
                sin * (topRadius1 + topRadius2 * .5f));
            vert += 2;
        }
        // Sides (out)
        ...
        while (vert < nbVerticesCap * 2 + nbVerticesSides) {
            ...
            vertices[vert] = new Vector3(cos * (topRadius1 + topRadius2 * .5f), height,
                sin * (topRadius1 + topRadius2 * .5f));
            vertices[vert + 1] = new Vector3(cos * (bottomRadius1 + bottomRadius2 * .5f), 0,
                sin * (bottomRadius1 + bottomRadius2 * .5f));
            vert += 2;
        }
        // Sides (in)
    ...
    public static GameObject CreateTorus() {
        var go = new GameObject("Torus_" + ++_torusID);
        var filter = go.AddComponent<MeshFilter>();
        Mesh mesh = filter.mesh;
        mesh.Clear();
        float radius1 = 1f;
    ...
    public static GameObject CreateSphere() {
        var go = new GameObject("Sphere_" + ++_sphereID);
